...
public class MainView extends VerticalLayout {
    ...
    private final Tab tabLicense = createTab("LICENSE & USE", VaadinIcon.DIPLOMA);
    private final Tab tabArchive = createTab("ARCHIVE", VaadinIcon.ARCHIVE);
    private final Tab tabConsole = createTab("CONSOLE", VaadinIcon.PLAY_CIRCLE);
    ...
    public MainView() {
        ...
        if (todayFile != null && todayFile.exists()) {
            ...
            consoleOutput.setValue(
            		  "All data generated by this web application are licensed under the Creative Commons Attribution 4.0 license (http://creativecommons.org/licenses/by/4.0). Any public use, publication, or official report MUST cite: "
            		...
            		+ "----------------------------------------------------------------------------------------------------\n"
            		+ "NOTE: Latest IMSR data extraction (up to 2026 August 31) can be downloaded from the ARCHIVE tab. You may just need to download several latest IMSR PDFs to update.\n"
                    + "----------------------------------------------------------------------------------------------------\n"
    ...
    private VerticalLayout buildLicenseView() {
        ...
        H2 header = new H2("Software License & Data Use");
        header.getStyle()
                .set("font-size", "1.5rem")
                .set("font-weight", "700")
                .set("color", "#0f172a")
        ...
        String htmlContent = "<b>SOFTWARE LICENSE (GNU GPLv3):</b><br>"
        		+ "Copyright (C) 2026 IMSR-WEBAPP DEVELOPER<br><br>"
                ...
                + "along with IMSR-WEBAPP. If not, see <a href=\"https://www.gnu.org/licenses/\" target=\"_blank\" style=\"color: #0284c7; text-decoration: underline;\">https://www.gnu.org/licenses/</a>.<br><br>"
                + "----------------------------------------------------------------------------------------------------<br>"
                + "<b>DATA USE & MANDATORY CITATION (CC-BY 4.0):</b><br>"
                + "All data generated by this web application are licensed under the Creative Commons Attribution 4.0 license "
                + "(<a href=\"https://creativecommons.org/licenses/by/4.0\" target=\"_blank\" style=\"color: #0284c7; text-decoration: underline;\">https://creativecommons.org/licenses/by/4.0</a>). "
                + "Any public use, publication, or official report <b>MUST</b> cite:<br><br>"
                + "Nguyen, D., Belval, E.J., Wei, Y. et al. Dataset of United States Incident Management Situation Reports from 2007 to 2021. "
                + "<i>Sci Data</i> 11, 23 (2024). <a href=\"https://doi.org/10.1038/s41597-023-02876-8\" target=\"_blank\" style=\"color: #0284c7; text-decoration: underline;\">https://doi.org/10.1038/s41597-023-02876-8</a>";
    ...
    private VerticalLayout buildArchiveView() {
        ...
        H2 header = new H2("Latest Extraction Archives");
        header.getStyle()
                .set("font-size", "1.5rem")
                .set("font-weight", "700")
                .set("color", "#0f172a")
        ...
        String htmlContent =
                ...
                + "----------------------------------------------------------------------------------------------------<br>"
                + "<b>DATA USE & MANDATORY CITATION (CC-BY 4.0):</b><br>"
                + "Above archives and all data generated by this web application are licensed under the Creative Commons Attribution 4.0 license "
                + "(<a href=\"https://creativecommons.org/licenses/by/4.0\" target=\"_blank\" style=\"color: #0284c7; text-decoration: underline;\">https://creativecommons.org/licenses/by/4.0</a>). "
                + "Any public use, publication, or official report <b>MUST</b> cite:<br><br>"
                + "Nguyen, D., Belval, E.J., Wei, Y. et al. Dataset of United States Incident Management Situation Reports from 2007 to 2021. "
                + "<i>Sci Data</i> 11, 23 (2024). <a href=\"https://doi.org/10.1038/s41597-023-02876-8\" target=\"_blank\" style=\"color: #0284c7; text-decoration: underline;\">https://doi.org/10.1038/s41597-023-02876-8</a>";
    ...
    private void runAggregationOnFiles(File[] pdfFiles) {
        // 1. Validate, rename, and clean files first.
        pdfFiles = prepareAndCleanFiles(pdfFiles);

        // 2. Immediate halt if validation failed or no valid files exist
        ...
        try {
            if (pdfFiles != null && pdfFiles.length > 0) {
                ...
                if (rawFolder.exists() && simple2Folder.exists()) {
                    ...
                    logBuilder.append("All data generated by this web application are licensed under the Creative Commons Attribution 4.0 license (http://creativecommons.org/licenses/by/4.0). Any public use, publication, or official report MUST cite: ");
        ...
        logBuilder.append("----------------------------------------------------------------------------------------------------\n");
        logBuilder.append("Export buttons can be found in the next 4 tabs: National Activity, Gacc Activity,...\n");
        logBuilder.append("----------------------------------------------------------------------------------------------------\n");
    ...
    private File[] prepareAndCleanFiles(File[] pdfFiles) {
        ...
        // 1. First Pass: Validate all file formats
        ...
        // 2. If at least 1 file has an invalid format, stop processing and show Pop-up
        if (!invalidFiles.isEmpty()) {
            showInvalidFilesDialog(invalidFiles);
            return null; // Signals runAggregationOnFiles to halt processing immediately
        }

        // 3. Second Pass: Process and rename files if all are valid
        for (File file : pdfFiles) {
            try {
                String fileName = file.getName();
                String dateKey = extractDateString(fileName); 
	...
	private String extractDateString(String fileName) {
	    ...
	    // Return null if no valid date pattern was found
	    return null;
	}

	private void showInvalidFilesDialog(List<String> invalidFiles) {
	    Dialog dialog = new Dialog();
	    dialog.setHeaderTitle("Invalid File Name Format (" + invalidFiles.size() + " files)");
	    ...
	    Span message = new Span("PDFs must have valid naming formats (...IMSR...MMDDYYYY.pdf or YYYYMMDDIMSR.pdf).\n"
	            + "Example of valid names: IMSR_CY26_08312026.pdf, 20260831IMSR.pdf\n"
	            + "Example of invalid names: sitrep.pdf, 2026IMSR.pdf\n\n"
	            + "Following PDFs need to be renamed to match the expected format:");
	    ...
	    fileList.getStyle()
	            ...
	            .set("max-height", "300px")       // Scroll panel height limit
	            .set("overflow-y", "auto")        // Enables scrolling for 100+ items
	            .set("width", "100%");
